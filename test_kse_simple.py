#!/usr/bin/env python3
"""
KSE Memory System Cross-Microservice Integration Test (Simple Version)
Tests the Next Phase capabilities without Unicode characters for Windows compatibility
"""
...
class KSECrossServiceTester:
    ...
    async def test_service_health(self) -> Dict[str, bool]:
        ...
        print(f"  Services healthy: {healthy_count}/{len(results)}")
        return results
    ...
    async def test_memory_storage_and_retrieval(self) -> bool:
        ...
        test_memories = [
            {
                "content": "User engagement analysis shows 85% retention rate for new feature",
                "context": {
                    "source": "surfacing",
                    "type": "analysis",
                    "timestamp": time.time()
        ...
        # Test retrieval via search
        try:
            search_query = {
                "query": "analysis retention conversion",
                "search_type": "hybrid",
                "limit": 10
            }
            ...
            async with self.session.post(
                ...
                if resp.status == 200:
                    ...
                    print(f"  [OK] Retrieved {len(results)} memories via hybrid search")

                    # Verify we can find memories from different sources
                    sources = [r.get('context', {}).get('source') for r in results]
                    unique_sources = set(filter(None, sources))
                    print(f"  [INFO] Found memories from sources: {unique_sources}")

                    return len(results) >= len(test_memories)
                else:
                    print(f"  [FAIL] Search failed: HTTP {resp.status}")
                    return False
        except Exception as e:
    ...
    async def run_basic_test(self) -> Dict[str, Any]:
        """Run basic KSE cross-microservice tests"""
        ...
        if not health_results.get('memory', False):
            print("\n[ABORT] Memory service not healthy - aborting tests")
        ...
        test_results = [results['memory_sharing']]
        ...
        if results['overall_success']:
            print("[SUCCESS] All KSE cross-microservice tests passed!")
        else:
...
async def main():
    ...
    try:
        async with KSECrossServiceTester() as tester:
            results = await tester.run_basic_test()
